...
python3 -V** to check the Python version number. _(Default as of this writing is 3.6.5.)_
    - _Take note!: Your entry points to run Python are 
...
3. Confirm docker is working (in Windows) by executing the following from command line: **
...
docker version** to confirm the docker version number _(As of this writing: **

18.09.2**)_
        - If this fails, see WSL install instructions above, or run **
...
docker run -it hello-world**
        - _(The first time you run this, it should fail with the message: "

Cannot connect to the Docker daemon at...")_
...
**/c/**. This is needed so that paths expressed in Linux can still be "found" by the Docker server running in Windows. (You will want to run docker from inside /c/ and not from /mnt/c/.)
        1. Either edit 

**/etc/wsl.conf** to look like this:
**[automount]
root = /
options = "metadata"**
        2. Or run: 
...
docker run -it hello-world**_ (this time it should work!)_
        - If hello-world runs, great! If not, that's also okay! Double check **step 5 **and **step 7d** above.
